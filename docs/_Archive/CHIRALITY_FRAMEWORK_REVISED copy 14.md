...
Using the example of Matrix C construction (applicable to all semantic dot product matrix construction)
for i over the ordered row indices
for j over the ordered column indices
do:
...
[C] = [(A(i,1) * B(1,j)) , (A(i,2) * B(2,j)) , (A(i,3) * B(3,j)) , (A(i,4) * B(4,j))]
...
1.	Evaluate only the operators. Apply the semantic rules to the expression exactly as written, following the stated order of operations (parentheses → * left-to-right → + left-to-right).
2.	Return a flat ordered list. The result must be a single bracketed sequence like [t1, t2, …] in the same left-to-right evaluation order (no nesting).
3.	No extra synthesis. Do not infer a combined label for the whole cell, do not explain it, and do not add, remove, or reorder items—only compute what the operators produce.
4.	No unresolved operators in the output. The output must contain none of: *, +, ⊙, ·, Σ, or transformation operators (transpose/truncate). Only the resulting ordered list items may remain.
...
expands to:
[D] = [
  [A(1,1) + ("resolution" * F(1,1)), A(1,2) + ("resolution" * F(1,2)), A(1,3) + ("resolution" * F(1,3)), A(1,4) + ("resolution" * F(1,4))],
  [A(2,1) + ("resolution" * F(2,1)), A(2,2) + ("resolution" * F(2,2)), A(2,3) + ("resolution" * F(2,3)), A(2,4) + ("resolution" * F(2,4))],
  [A(3,1) + ("resolution" * F(3,1)), A(3,2) + ("resolution" * F(3,2)), A(3,3) + ("resolution" * F(3,3)), A(3,4) + ("resolution" * F(3,4))]
]
```
...
To construct [G], retain from [X] the rows [guiding, applying, judging]; drop [reflecting]
