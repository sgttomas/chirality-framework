...
class TestPrerequisiteMatrices:
    ...
    def test_compute_array_P(self):
        ...
        assert (
            validation_errors == []
        ), f"P should pass validation, but got errors: {validation_errors}"
...
class TestMatrixEComputation:
    ...
    def test_compute_cell_E(self):
        ...
        # Verify Stage 2 has resolved concepts (new API returns single combined text)
        stage2_text = cell.provenance["stage_2_semantic"]["text"]
        assert isinstance(stage2_text, str)
        assert len(stage2_text) > 0  # Combined resolved concepts in single text
    ...
    def test_E_provenance_validation(self):
        ...
        assert (
            validation_errors == []
        ), f"E cell should pass validation, but got errors: {validation_errors}"
...
class TestResolverCallCounts:
    ...
    def test_E_resolver_call_count(self):
        ...
        # E uses full 3-stage pipeline like C and X (but with combined lensing)
        assert (
            call_counts["run_stage2_multiply"] == 1
        ), "Should call run_stage2_multiply once with all k-products"
        assert call_counts["run_combined_lens"] == 1, "Should call combined lens once"

        # Station shift should not be called for E
        assert call_counts["run_shift"] == 0, "E should not use station shift"
