...
class TestMatrixZComputation:
    ...
    def test_compute_cell_Z(self):
        ...
        # Verify lean 2-stage structure (stages 3-5 should be empty for Z)
        ...
        assert cell.provenance["stage_3_column_lensed"] == {}
        assert cell.provenance["stage_4_row_lensed"] == {}
        assert cell.provenance["stage_5_final_synthesis"] == {}
        ...
        assert "VALIDATION" in stage2_text  # Should have validation context from mock
    ...
    def test_Z_provenance_validation(self):
        ...
        assert validation_errors == [], f"Z cell should pass validation, but got errors: {validation_errors}"
    ...
    def test_Z_dimensions_and_station_shift(self):
        ...
        for i in range(2):  # Check first 2 rows
            for j in range(2):  # Check first 2 columns
                ...
                assert z_cell.value != x_cell.value, f"Z[{i},{j}] should be different from X[{i},{j}] due to station shift"
    ...
    def test_resolver_call_count(self):
        ...
        assert call_counts["shift_station_context"] == 1, "Should call shift_station_context once per Z cell"
        
        # Other methods should not be called for Z (lean 2-stage)
        assert call_counts["apply_column_lens"] == 0, "Z should not use column lens"
        assert call_counts["apply_row_lens"] == 0, "Z should not use row lens"
        assert call_counts["synthesize_lensed_perspectives"] == 0, "Z should not use synthesis"