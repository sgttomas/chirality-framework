...
def test_cache_invalidation_on_snapshot_change(temp_cache):
    ...
    cache_key1 = temp_cache.compute_cache_key(
        **base_params,
        snapshot_hash="phase1_snapshot_v1"
    )
    ...
    cache_key2 = temp_cache.compute_cache_key(
        **base_params,
        snapshot_hash="phase1_snapshot_v2"  # Changed
    )
...
def test_cache_invalidation_on_lens_catalog_change(temp_cache):
    ...
    cache_key1 = temp_cache.compute_cache_key(
        **base_params,
        lens_catalog_digest="lens_catalog_v1"
    )
    ...
    cache_key2 = temp_cache.compute_cache_key(
        **base_params,
        lens_catalog_digest="lens_catalog_v2"  # Changed
    )
...
def test_cache_invalidation_on_gpt5_params(temp_cache):
    ...
    cache_key3 = temp_cache.compute_cache_key(
        **base_params,
        verbosity="medium",
        reasoning_effort="minimal",  # Changed
        max_tokens=1000
    )
...
def test_cache_key_collision_resistance(temp_cache):
    ...
    for i in range(10):
        for j in range(10):
            key = temp_cache.compute_cache_key(
                **base_params,
                indices=(i, j)
            )
