        ...
        for line in f:
            ...
            if line:
                ...
                if len(cells) != expected_cells:
                    print(f"Warning: Shape {shape} expects {expected_cells} cells, but found {len(cells)}")
...
def render_page(snapshot_data_by_matrix: Dict[str, Dict], run_id: str, title: Optional[str] = None) -> str:
    """
    ...
    html_parts.append(f'<div class="run-info">Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</div>')
    ...
    for matrix_name in CANONICAL_ORDER:
        ...
        if matrix_name in snapshot_data_by_matrix:
            ...
            html_parts.append(f'<p><strong>Shape:</strong> {shape[0]}×{shape[1]} ({cell_count} cells)</p>')
...
def render_elements_page(snapshot_data_by_matrix: Dict[str, Dict], run_id: str, 
                        title: Optional[str] = None, sanitize: bool = False) -> str:
    """
...
def find_matrix_snapshots(run_dir: Path, include_matrices: Optional[List[str]] = None) -> Dict[str, Path]:
    """
...
def load_snapshots_for_run(run_dir: Path, include_matrices: Optional[List[str]] = None) -> Dict[str, Dict]:
    """
