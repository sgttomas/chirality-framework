...
def cmd_assets_verify(args):
    ...
    log_success(f"Manifest created: kernel_hash={result['kernel_hash']}, assets={result['asset_count']}")
...
def cmd_phase1_dialogue_run(args):
    ...
    if budget_config:
        log_info(f"  Budget limits: tokens={args.token_budget}, cost=${args.cost_budget}, time={args.time_budget}s")
...
def cmd_phase2_run(args):
    ...
    log_info(f"  Model: {args.model} (temp={args.temperature}, top_p={getattr(args, 'top_p', args.__dict__.get('top_p', 0.9))})")
    ...
    for tensor_spec_item in tensor_spec.get("tensors", []):
        ...
        try:
            ...
            if budget_config:
                ...
                if budget_status:
                    log_info(f"  Budget status: {budget_status['tokens']['total']:,} tokens, ${budget_status['cost']['spent']:.4f}")
            raise
    ...
    if budget_config:
        ...
        budget_status = engine.get_budget_status()
        log_success(f"Budget status: {budget_status['tokens']['total']:,} tokens, ${budget_status['cost']['spent']:.4f} USD")
...
def main():
    ...
    p1_snap.add_argument("--from", dest="dialogue", required=True, help="Path to phase1_dialogue.jsonl")
    ...
    p2_run.add_argument("--out", default="artifacts/", help="Output directory for caching and resume")
    ...
    p2_run.add_argument("--cache", action="store_true", default=True, help="Enable caching (default: True)")
