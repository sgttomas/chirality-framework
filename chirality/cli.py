...
def resolve_run_settings(trace: bool, neo4j_export: bool, trace_only: bool) -> tuple[bool, bool]:
    ...
    # Precedence 2: CHIRALITY_DISABLE_EXPORT environment variable
        ...
        if disable_export_env in {'1', 'true', 'yes', 'on'}:
            if neo4j_export:
                click.echo(click.style("Neo4j export disabled by CHIRALITY_DISABLE_EXPORT environment variable.", **INFO_STYLE))
            final_neo4j_export = False
...
@cli.command()
@click.argument('matrix', type=click.Choice(['C', 'F', 'D', 'K', 'X', 'Z', 'G', 'P', 'T', 'E'], case_sensitive=False))
@click.option('--i', 'row', type=int, required=True, 
              help='Row index')
@click.option('--j', 'col', type=int, required=True,
              help='Column index')
...
@click.option('--trace-only', is_flag=True,
              help='Enable tracing and force-disable Neo4j export.')
def compute_cell(matrix: str, row: int, col: int, verbose: bool, 
                resolver: str, api_key: Optional[str], trace: bool, neo4j_export: bool, trace_only: bool):
    """
    ...
    try:
        ...
        if matrix.upper() == 'C':
            ...
            if row < 0 or row >= 4:
                click.echo(click.style(f"Error: Row index {row} out of range for K (0-3)", **ERROR_STYLE))
                sys.exit(1)
            if col < 0 or col >= 3:
                click.echo(click.style(f"Error: Column index {col} out of range for K (0-2)", **ERROR_STYLE))
                sys.exit(1)
            ...
            valley_summary = "Problem Statement -> Problem Requirements -> Solution Objectives -> [Verification]"
            ...
            if row < 0 or row >= 4:
                click.echo(click.style(f"Error: Row index {row} out of range for X (0-3)", **ERROR_STYLE))
                sys.exit(1)
            if col < 0 or col >= 4:
                click.echo(click.style(f"Error: Column index {col} out of range for X (0-3)", **ERROR_STYLE))
                sys.exit(1)
            ...
            if verbose:
                ...
                # Show semantic resolution (simplified for CLI display)
                click.echo("  Resolving semantic products...")
                
                click.echo()
                click.echo(click.style("STAGE 3: Ontological Lensing", **STAGE_STYLE))
                click.echo(click.style("-" * 40, **DIM_STYLE))
                click.echo("  Applying universal 3-step lensing...")
            
            cell = compute_cell_X(row, col, K, J, resolver_obj, valley_summary, tracer_obj, exporter_obj)
            ...
            if row < 0 or row >= 4:
                click.echo(click.style(f"Error: Row index {row} out of range for Z (0-3)", **ERROR_STYLE))
                sys.exit(1)
            if col < 0 or col >= 4:
                click.echo(click.style(f"Error: Column index {col} out of range for Z (0-3)", **ERROR_STYLE))
                sys.exit(1)
            ...
            if verbose:
                ...
                click.echo()
                click.echo(click.style("STAGE 2: Station Shift (Verification → Validation)", **STAGE_STYLE))
                click.echo(click.style("-" * 40, **DIM_STYLE))
                click.echo("  Applying station context shift...")
            
            cell = compute_cell_Z(row, col, X, resolver_obj, valley_summary, tracer_obj, exporter_obj)
            ...
            if row < 0 or row >= 3:
                click.echo(click.style(f"Error: Row index {row} out of range for G (0-2)", **ERROR_STYLE))
                sys.exit(1)
            if col < 0 or col >= 4:
                click.echo(click.style(f"Error: Column index {col} out of range for G (0-3)", **ERROR_STYLE))
                sys.exit(1)
            ...
            if row != 0:
                click.echo(click.style(f"Error: Row index {row} out of range for P (0 only)", **ERROR_STYLE))
                sys.exit(1)
            if col < 0 or col >= 4:
                click.echo(click.style(f"Error: Column index {col} out of range for P (0-3)", **ERROR_STYLE))
                sys.exit(1)
            
            if verbose:
                click.echo()
                click.echo(click.style("P = Z[3,:] (Fourth row of Validation as 1x4)", **STAGE_STYLE))
                click.echo(click.style("-" * 40, **DIM_STYLE))
            ...
            if row < 0 or row >= 4:
                click.echo(click.style(f"Error: Row index {row} out of range for T (0-3)", **ERROR_STYLE))
                sys.exit(1)
            if col < 0 or col >= 3:
                click.echo(click.style(f"Error: Column index {col} out of range for T (0-2)", **ERROR_STYLE))
                sys.exit(1)
            
            if verbose:
                click.echo()
                click.echo(click.style("T = (B[0:3,:])^T (Transpose of first 3 rows of B)", **STAGE_STYLE))
                click.echo(click.style("-" * 40, **DIM_STYLE))
            ...
            if row < 0 or row >= 3:
                click.echo(click.style(f"Error: Row index {row} out of range for E (0-2)", **ERROR_STYLE))
                sys.exit(1)
            if col < 0 or col >= 3:
                click.echo(click.style(f"Error: Column index {col} out of range for E (0-2)", **ERROR_STYLE))
                sys.exit(1)
            ...
            cell = compute_cell_E(row, col, G, T, resolver_obj, valley_summary, tracer_obj, exporter_obj)
...
@cli.command()
@click.argument('matrix', type=click.Choice(['A', 'B', 'J', 'C', 'F', 'D', 'K', 'X', 'Z', 'G', 'P', 'T', 'E'], case_sensitive=False))
@click.option('--resolver', type=click.Choice(['echo', 'openai'], case_sensitive=False),
              default='echo', help='Resolver to use (default: echo for testing)')
@click.option('--api-key', envvar='OPENAI_API_KEY',
              help='OpenAI API key (or set OPENAI_API_KEY env var)')
...
@click.option('--trace-only', is_flag=True,
              help='Enable tracing and force-disable Neo4j export.')
@click.option('--snapshot-jsonl', is_flag=True,
              help='Save a JSONL snapshot of the final matrix')
@click.option('--verbose', '-v', is_flag=True,
              help='Show progress messages')
def compute_matrix(matrix: str, resolver: str, api_key: Optional[str], trace: bool, 
                  neo4j_export: bool, trace_only: bool, snapshot_jsonl: bool, verbose: bool):
    """
    ...
    try:
        ...
        run_id = f"matrix-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:6]}"
        ...
        if matrix_upper == 'A':
            ...
            result_matrix = compute_matrix_C(A, B, resolver_obj, valley_summary, tracer_obj, exporter_obj)
        elif matrix_upper == 'F':
            if verbose:
            ...
            result_matrix = compute_matrix_F(J, C, resolver_obj, valley_summary, tracer_obj, exporter_obj)
        elif matrix_upper == 'D':
            if verbose:
            ...
            result_matrix = compute_matrix_D(A, F, resolver_obj, valley_summary, tracer_obj, exporter_obj)
        elif matrix_upper == 'K':
            if verbose:
            ...
            result_matrix = compute_matrix_X(K, J, resolver_obj, valley_summary, tracer_obj, exporter_obj)
        elif matrix_upper == 'Z':
            if verbose:
            ...
            result_matrix = compute_matrix_Z(X, resolver_obj, valley_summary, tracer_obj, exporter_obj)
        elif matrix_upper == 'G':
            if verbose:
            ...
            result_matrix = compute_matrix_E(G, T, resolver_obj, valley_summary, tracer_obj, exporter_obj)
        
        # Display summary
        if result_matrix:
            click.echo()
            click.echo(click.style(f"Matrix {matrix.upper()} Computed Successfully", **SUCCESS_STYLE))
            click.echo(click.style("-" * 40, **DIM_STYLE))
            ...
            click.echo(f"  Row Labels: {', '.join(result_matrix.row_labels[:3])}..." if len(result_matrix.row_labels) > 3 else f"  Row Labels: {', '.join(result_matrix.row_labels)}")
            click.echo(f"  Column Labels: {', '.join(result_matrix.col_labels[:3])}..." if len(result_matrix.col_labels) > 3 else f"  Column Labels: {', '.join(result_matrix.col_labels)}")
...
@click.option('--trace-only', is_flag=True,
              help='Enable tracing and force-disable Neo4j export.')
@click.option('--snapshot-jsonl', is_flag=True,
              help='Save JSONL snapshots of computed matrices')
@click.option('--include-base', is_flag=True,
              help='Include base matrices (A, B, J) in the pipeline')
@click.option('--only', 
        ...
        for matrix_name in matrices_to_compute:
            ...
            if matrix_name == 'A':
                ...
                if 'C' not in computed_matrices:
                    computed_matrices['C'] = compute_matrix_C(A, B, resolver_obj, valley_summary, tracer_obj, exporter_obj)
                result_matrix = computed_matrices['C']
            elif matrix_name == 'F':
                if 'C' not in computed_matrices:
                    computed_matrices['C'] = compute_matrix_C(A, B, resolver_obj, valley_summary, tracer_obj, exporter_obj)
                if 'F' not in computed_matrices:
                    computed_matrices['F'] = compute_matrix_F(J, computed_matrices['C'], resolver_obj, valley_summary, tracer_obj, exporter_obj)
                ...
                if 'D' not in computed_matrices:
                    computed_matrices['D'] = compute_matrix_D(A, computed_matrices['F'], resolver_obj, valley_summary, tracer_obj, exporter_obj)
                ...
                if 'X' not in computed_matrices:
                    computed_matrices['X'] = compute_matrix_X(computed_matrices['K'], J, resolver_obj, valley_summary, tracer_obj, exporter_obj)
                ...
                if 'Z' not in computed_matrices:
                    computed_matrices['Z'] = compute_matrix_Z(computed_matrices['X'], resolver_obj, valley_summary, tracer_obj, exporter_obj)
                ...
                if 'E' not in computed_matrices:
                    computed_matrices['E'] = compute_matrix_E(computed_matrices['G'], computed_matrices['T'], resolver_obj, valley_summary, tracer_obj, exporter_obj)
...
@cli.command()
@click.option('--run-id', 
              help='Specific run ID to render (e.g., "pipeline-20250903-214419-ec5359")')
@click.option('--latest', is_flag=True,
              help='Use the most recent run directory')
@click.option('--source-dir', type=click.Path(exists=True, file_okay=False, dir_okay=True),
              default='snapshots', help='Directory containing snapshot runs (default: snapshots)')
@click.option('--output-dir', type=click.Path(file_okay=False, dir_okay=True),
              default='viewer-output', help='Directory to write HTML files (default: viewer-output)')
@click.option('--title',
              help='Custom title for the HTML page')
@click.option('--include',
              help='Include only specific matrices (comma-separated list, e.g., "A,C,F")')
@click.option('--style', type=click.Choice(['tables', 'elements'], case_sensitive=False),
              default='tables', help='Display style: tables (default) or elements')
@click.option('--no-sanitize-values', is_flag=True,
...
def render_viewer(run_id: Optional[str], latest: bool, source_dir: str, output_dir: str,
                 title: Optional[str], include: Optional[str], style: str, 
                 no_sanitize_values: bool, open_browser: bool):
    """
    ...
    try:
        ...
        if run_id and latest:
            click.echo(click.style("Error: Cannot specify both --run-id and --latest", **ERROR_STYLE))
            sys.exit(1)
        elif run_id:
            # Validate specified run ID exists
            run_dir = snapshots_dir / run_id
            if not run_dir.exists():
                click.echo(click.style(f"Error: Run directory '{run_id}' not found in {snapshots_dir}", **ERROR_STYLE))
                sys.exit(1)
            ...
            if not target_run_id:
                click.echo(click.style(f"Error: No run directories found in {snapshots_dir}", **ERROR_STYLE))
                sys.exit(1)
        else:
            click.echo(click.style("Error: Must specify either --run-id or --latest", **ERROR_STYLE))
            sys.exit(1)
        ...
        if not snapshot_data_by_matrix:
            click.echo(click.style(f"Error: No valid snapshot files found in {run_dir}", **ERROR_STYLE))
            sys.exit(1)
        ...
        if style.lower() == 'elements':
            ...
            html_content = render_elements_page(snapshot_data_by_matrix, target_run_id, title, sanitize)
        else:
...
def info():
    ...
    click.echo(f"      Columns:{' ' if MATRIX_A.row_labels else ''}{', '.join(MATRIX_A.row_labels)}")  # Transposed from D rows
    # X axes are K rows × J columns
    click.echo("    X Axes:")
    click.echo(f"      Station: Verification")
    click.echo(f"      Rows:   {', '.join(MATRIX_A.col_labels)}")  # K row labels
    click.echo(f"      Columns:{' ' if MATRIX_J.col_labels else ''}{', '.join(MATRIX_J.col_labels)}")  # J column labels
    ...
    click.echo(f"      Columns:{' ' if MATRIX_J.col_labels else ''}{', '.join(MATRIX_J.col_labels)}")  # Same as X
    ...
    click.echo(f"      Columns:{' ' if MATRIX_J.col_labels else ''}{', '.join(MATRIX_J.col_labels)}")  # Same as Z
    ...
    click.echo(f"      Columns:{' ' if MATRIX_B.row_labels[:3] else ''}{', '.join(MATRIX_B.row_labels[:3])}")  # First 3 B rows become T columns
    ...
    click.echo(f"      Rows:   {', '.join(MATRIX_A.col_labels[:3])}")  # G row labels (first 3 from Z)
    click.echo(f"      Columns:{' ' if MATRIX_B.row_labels[:3] else ''}{', '.join(MATRIX_B.row_labels[:3])}")  # T column labels
    click.echo()
