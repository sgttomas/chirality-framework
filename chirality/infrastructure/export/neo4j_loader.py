...
class Neo4jLoader:
        ...
        with self.driver.session() as session:
            for constraint in constraints:
                try:
                    ...
                    print(f"✓ Created constraint: {constraint.split(' FOR ')[1].split(' REQUIRE')[0]}")
                    ...
                    if "already exists" not in str(e).lower() and "equivalent" not in str(e).lower():
                        ...
                        print(f"✓ Constraint already exists: {constraint.split(' FOR ')[1].split(' REQUIRE')[0]}")
    ...
    def _create_run_graph(
        ...
        for lens in lens_catalog:
            ...
            tx.run("MERGE (l:Lens {lens_id: $lens_id}) SET l += $props", 
                  lens_id=lens["lens_id"], props=lens_props)
    ...
    def _create_matrix_component(
        ...
        CREATE (r)-[:CONTAINS]->(c)
        """, run_id=run_id, props=component_props)
        ...
        for i, row_elements in enumerate(elements):
            for j, value in enumerate(row_elements):
                ...
                """, run_id=run_id, name=name, cell_props=cell_props)
                ...
                if "lens_id" in cell_props:
                    ...
                    """, cell_id=cell_id, lens_id=cell_props["lens_id"])
        ...
        for idx_tuple, cell_data in cells.items():
            ...
            for source_id in sources:
                try:
                    ...
                    """, cell_id=cell_id, source_id=source_id)
                except Exception:
