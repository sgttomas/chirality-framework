...
class CellComputationService:
    ...
    def _execute_stage2(self, matrix_type: str, stage1_result):
        ...
        if matrix_type in ["C", "X", "E"]:
            return self.resolver.run_stage2_multiply(
                stage1_result["texts"], matrix_type
            )
        elif matrix_type == "F":
            return self.resolver.run_stage2_elementwise(
                stage1_result["texts"], matrix_type
            )
        elif matrix_type == "D":
            # Mechanical addition - no LLM call
            return {
                "text": " applied to frame the problem; ".join(stage1_result["texts"]) + " to resolve the problem.",
