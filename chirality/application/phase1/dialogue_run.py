...
"""
        ...
        lensed = self._call_llm_with_json_tail(
            user_msg,
            get_tail("C", "lensed"),
            "C_lensed"
        )
...
"""
        return self._call_llm_with_json_tail(
            user_msg,
            get_tail("J", "base"),
            "J_base"
        )
...
"""
    ...
    def _compute_matrix_z(self, X: Dict) -> tuple[Dict[str, Any], List[str]]:
        ...
        shifted = self._call_llm_with_json_tail(
            user_msg,
            get_tail("Z", "shifted"),
            "Z_shifted"
        )
    ...
    def _aggregate_results(
        ...
        aggregated, metadata = try_parse_json_or_repair(
            messages=self.dialogue_history + [{"role": "user", "content": f"{user_msg}\n\n{get_tail('aggregator', '')}"}],
            adapter_call=adapter_call,
            schema_hint=schema_hint,
            max_repair_attempts=self.max_repair
        )
