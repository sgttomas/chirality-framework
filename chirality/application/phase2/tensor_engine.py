...
class TensorEngine:
    ...
    def compute_tensor(
        ...
        for idx in cell_indices:
            ...
            cache_key = self._compute_cache_key(name, idx, left_source, right_source, matrix_for_lens)
    ...
    def _compute_tensor_cell(
        ...
        # Build user message
        user_message = self._build_cell_prompt(
            tensor_name, idx, left_value, right_value, lens_text
        )
...
"""
    ...
    def _get_cell_contract(self, tensor_name: str) -> str:
        ...
        return f"""Return JSON only: {{"tensor":"{tensor_name}","value":"...","confidence":0.0-1.0}}"""